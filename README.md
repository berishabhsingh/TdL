...
- **Integrated High-Performance Telegram Bot** powered by Pyrogram that downloads files via the gateway and sends them directly to users.
...
- **High-Performance Telegram Bot**:
  - Accept Terabox links via chat
  - Concurrency up to 100 parallel downloads and uploads using asyncio Semaphores
  - Configurable Dump Channel (files are uploaded to a channel first and then forwarded to user)
  - Seamless integration with the underlying TeraBox API gateway
...
You must also add credentials to `.env`:

```env
BOT_TOKEN=your_bot_token_from_botfather
BOT_API_ID=your_api_id_from_my_telegram_org
BOT_API_HASH=your_api_hash_from_my_telegram_org
DUMP_CHANNEL_ID=-1001234567890 # (Optional) dump downloaded files here
```

**No Cookies required out of the box!**
If you do not specify `COOKIE_JSON`, the bot and the API will automatically fall back to using the public TeraBox gateway (`tera-core.vercel.app`), making setup completely hassle-free.
*Note: The public gateway is shared and may encounter 403 Forbidden errors if rate-limited by TeraBox. To ensure maximum uptime, we highly recommend adding your own `COOKIE_JSON`.*

The API automatically detects the format and handles it accordingly.

### 2. Deploy the Telegram Bot using Docker (Recommended)

A `docker-compose.yml` configuration is provided for launching the Telegram bot quickly and isolating its environment. It's configured to set the system DNS to `1.1.1.1` to enhance connection reliability.
