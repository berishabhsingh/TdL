...
class siteList:
    ...
    TERA = ['terabox.com', 'nephobox.com', '4funbox.com', 'mirrobox.com', 'momerybox.com', 'teraboxapp.com', '1024tera.com', 'terabox.app', 'gibibox.com', 'terasharefile.com']
...
def rc(url: str):
    ...
    if len(details['contents']) == 0:
        raise DirectDownloadLinkException('ERROR: Direct link not found!')
    if len(details['contents']) == 1:
            return details['contents'][0]['url'], details['contents'][0]['filename']
    return details
...
def terabox(url):
    ...
    data = get_flowvideo_links(url)

    if isinstance(data, dict) and data.get("error"):
        raise DirectDownloadLinkException(f"ERROR: {data.get('error', 'Unknown API error')}")
    elif "data" in data:
        items = data["data"]
        for item in items:
            dl_url = item.get('stream_final_url') or item.get('download_url') or item.get('url')
            if not dl_url:
                continue
        ...
        raise DirectDownloadLinkException("ERROR: Could not retrieve links")
